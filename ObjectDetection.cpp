...
int ObjectDetection::contourThickness(cv::Mat image) {
    int rows = image.rows;
    int cols = image.cols;
...
cv::Mat ObjectDetection::readImage(const std::string& imgPath) {
    // Read the image
...
int ObjectDetection::getHSV(const cv::Mat srcImage) {
...
int ObjectDetection::getAverageHSV(const cv::Mat& image, int x, int y) {
    int rows = image.rows;
    int cols = image.cols;
    int scale = 2;
...
cv::Mat ObjectDetection::createMask(const cv::Mat& inputImage, const cv::Scalar& lowerBound, const cv::Scalar& upperBound) {
...
cv::Mat ObjectDetection::applyMask(const cv::Mat& image, const cv::Mat& mask) {
    cv::Mat resultImage;
...
cv::Mat ObjectDetection::identifyAllObjects(cv::Mat& image, int invert) {
...
int ObjectDetection::identifyAllObjectAreas(cv::Mat& image, int invert) {
...
void ObjectDetection::removeBackground(cv::Mat& image) {
...
cv::Mat ObjectDetection::identifyColor(cv::Mat image) {
    int range = 4;
    cv::Mat resultImage;
    cv::Mat mask;
...
cv::Mat ObjectDetection::identifyColor(cv::Mat image, int x, int y) {
    int range = 4;
    cv::Mat resultImage;
    cv::Mat mask;
