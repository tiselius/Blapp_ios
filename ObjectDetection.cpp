...
void ObjectDetection::writeContourMask(cv::Mat image) {
    // Convert the image to grayscale
    cv::Mat gray;
    cv::cvtColor(image, gray, cv::COLOR_BGR2GRAY);
...
void ObjectDetection::writeContour(cv::Mat image) {
    // Convert the image to grayscale
    cv::Mat gray;
    cv::cvtColor(image, gray, cv::COLOR_BGR2GRAY);
...
int ObjectDetection::getAverageHSV(const cv::Mat image) {
...
int ObjectDetection::getHSV(const cv::Mat image) {
    if (image.empty()) {
        std::cerr << "Error: Could not open the image file." << std::endl;
        return -1;
    }
...
cv::Mat ObjectDetection::identifyObject(cv::Mat image) {
    int range = 4;
    cv::Mat resultImage;
    cv::Mat mask;
...
int ObjectDetection::identifyObjectTest(cv::Mat image) {
    int range = 4;
    cv::Mat resultImage;
    cv::Mat mask;
